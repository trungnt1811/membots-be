...
func RegisConsumers(config *conf.Configuration, db *gorm.DB) {
	...
	fraudAlert := alert.NewAlertClient(config.Discord.AlertFraudUrl)
	...
	// Start consumer
	rewardMaker := NewRewardMaker(rewardRepo, orderRepo, tikiClientCache, orderApproveQueue, appNotiQueue, fraudAlert)
