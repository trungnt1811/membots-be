...
func (a *affBannerRepository) SearchWithStatus(ctx context.Context, q string, listStatus []string, page, size int) (model.AffSearch, error) {
	...
	err := a.Db.Table("aff_campaign").
		Joins("Brand").
		Where("aff_campaign.stella_status IN ? AND (MATCH (aff_campaign.name) AGAINST(? IN NATURAL LANGUAGE MODE) OR "+
			"MATCH (Brand.name) AGAINST(? IN NATURAL LANGUAGE MODE))", listStatus, fmt.Sprint(q, " *"), fmt.Sprint(q, " *")).
		Limit(size + 1).
		Offset(offset).
	...
	err = a.Db.Raw("select count(*) FROM aff_campaign as ac LEFT JOIN brand as b ON b.id = ac.brand_id "+
		"WHERE ac.stella_status IN ? AND (MATCH (ac.name) AGAINST(? IN NATURAL LANGUAGE MODE) OR "+
		"MATCH (b.name) AGAINST(? IN NATURAL LANGUAGE MODE))",
		listStatus, fmt.Sprint(q, " *"), fmt.Sprint(q, " *")).Scan(&totalAffCampaign).Error
	if err != nil {
		return searchResult, err
	}
