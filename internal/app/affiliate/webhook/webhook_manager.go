...
import (
	...
	"github.com/astraprotocol/affiliate-system/internal/app/affiliate/webhook/discord"
)
...
func NewWebHookManagerFromConfig(cfg conf.WebhookConfiguration) (*WebHookManager, error) {
	...
	discordConf := discord.DefaultConfig()
	discordConf.Token = cfg.DcConf.Token
	discordConf.SubChannels = cfg.DcConf.SubChannels

	whs["Discord"], err = discord.NewDiscordHook(discordConf)
	if err != nil {
